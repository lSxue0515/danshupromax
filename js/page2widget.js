...
/* =============================================
   拍立得轮播组件 — 动态切换 + 图片上传 + 持久化
   ============================================= */
var _p2CarOrder = [0, 1, 2];        // 当前位置顺序：[左, 中, 右]
var _p2CarClasses = ['p2-cc-left', 'p2-cc-center', 'p2-cc-right'];
var _p2CarTimer = null;

function _p2CarApply() {
    var cards = document.querySelectorAll('.p2-carousel-card');
    if (cards.length !== 3) return;
    for (var i = 0; i < 3; i++) {
        var card = null;
        for (var j = 0; j < cards.length; j++) {
            if (parseInt(cards[j].dataset.slot) === _p2CarOrder[i]) {
                card = cards[j];
                break;
            }
        }
        if (!card) continue;
        card.className = 'p2-carousel-card ' + _p2CarClasses[i];
    }
}

function _p2CarNext() {
    // 循环：左→中→右→左
    _p2CarOrder = [_p2CarOrder[2], _p2CarOrder[0], _p2CarOrder[1]];
    _p2CarApply();
}

function _p2CarStartAuto() {
    _p2CarStopAuto();
    _p2CarTimer = setInterval(_p2CarNext, 2000);
}

function _p2CarStopAuto() {
    if (_p2CarTimer) { clearInterval(_p2CarTimer); _p2CarTimer = null; }
}

/* 上传图片 */
function p2PickCarousel(slot) {
    _p2CarStopAuto();
    _p2PickFile(function (result) {
        var img = document.getElementById('p2CarImg' + slot);
        if (img) img.src = result;
        _p2Load();
        _p2Data['carImg' + slot] = result;
        _p2Save();
        _p2CarStartAuto();
    }, 300, 300);
    // 如果用户取消了也要重启
    setTimeout(function () { _p2CarStartAuto(); }, 500);
}
...
function _p2CarInit() {
    ...
    for (var i = 0; i < 3; i++) {
        var key = 'carImg' + i;
        if (_p2Data[key]) {
            var img = document.getElementById('p2CarImg' + i);
            if (img) img.src = _p2Data[key];
        }
    }
    _p2CarApply();
    _p2CarStartAuto();
